...
export function StoragePoolCreator({
  ...
  const cacheCandidates = useMemo(() => {
    return raidCandidates.filter((candidate) => {
      const transport = String(candidate.transport || '').toLowerCase()
      const model = String(candidate.model || '').toLowerCase()
      const isFlash =
        transport === 'nvme' || model.includes('ssd') || model.includes('nvme')
      return (
        candidate.eligible &&
        isFlash &&
        !selectedDiskIds.includes(candidate.path)
      )
    })
  }, [raidCandidates, selectedDiskIds])
    ...
    <div className="flex min-h-full flex-col">
      <div className="flex-1 space-y-6 p-4 select-text">
        {submitError && (
          <div
            ref={errorRef}
            className="rounded-md border border-red-500/40 bg-red-500/12 px-3 py-2 text-xs text-red-300"
          >
            {submitError}
          </div>
        )}
